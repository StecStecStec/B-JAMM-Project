...
public class CLI {
  ...
  private String createLibrarian(List<String> arguments, Library library) {
    ...
    while (i < library.getLibrarianList().size()) {
      if (Objects.equals(library.getLibrarianList().get(i).getLibrarianName(), name)
          && Objects.equals(library.getLibrarianList().get(i).getLibrarianSurname(), surname)) {
  ...
  private String deleteLibrarian(List<String> arguments, Library library) {
    ...
    while (i < library.getLibrarianList().size()) {
      if (Objects.equals(library.getLibrarianList().get(i).getLibrarianName(), name)
          && Objects.equals(library.getLibrarianList().get(i).getLibrarianSurname(), surname)
          && Objects.equals(library.getLibrarianList().get(i).getLibrarianBirthday(), birthday)) {
  ...
  private String borrowBook(List<String> arguments, Library library) {
    ...
    int j = 0;
    while (i < library.getBookList().size()) {
      while (j < library.getVisitorList().size()
          && Objects.equals(library.getBookList().get(i).getBookID(), uuid)) {
  ...
  private String returnBook(List<String> arguments, Library library) {
    ...
    UUID uuid = UUID.fromString(arguments.get(1));
    while (i < library.getBookList().size()) {
      while (j < library.getShelfList().size()
          && library.getBookList().get(i).getBookID().equals(uuid)) {
        if (Objects.equals(
            library.getShelfList().get(j).getGenre(),
            library.getBookList().get(i).getBookGenre())) {
  ...
  private String viewBorrowedBooks(Library library) {
    ...
    while (i < library.getBookList().size()) {
      if (library.getBookList().get(i).getBorrowedBy() != null
          && library.getBookList().get(i).getShelf() == null) {
        out.print(library.getBookList().get(i).getBookID() + "\t");
        out.print(library.getBookList().get(i).getBookTitle() + "\t");
        out.print(library.getBookList().get(i).getBookAuthor() + "\t");
        out.print(library.getBookList().get(i).getBookGenre() + "\t");
        out.print(
            library.getBookList().get(i).getBorrowedBy().getVisitorName()
                + " "
                + library.getBookList().get(i).getBorrowedBy().getVisitorSurname()
                + "\t");
  ...
  public String check(List<String> arguments) {
    ...
    String options =
        "createVisitor, createLibrarian, createShelf, deleteVisitor, deleteLibrarian, deleteShelf, addBook, viewBooks, deleteBook, searchBook, borrowBook, returnBook, restoreBook, viewBorrowedBooks";
    return switch (arguments.getFirst()) {
      ...
      case CREATE_SHELF ->
          (arguments.size() != 5)
              ? "Usage: [option] [genre] [shelfWidth] [boardNumber] [Folder]\n" + options
              : null;
      case CREATE_LIBRARIAN, DELETE_LIBRARIAN ->
          (arguments.size() != 5)
              ? "Usage: [option] [Name] [Surname] [Birthday] [Folder]\n" + options
              : null;
      case CREATE_ROOM ->
          (arguments.size() != 3) ? "Usage: [option] [shelfLimit] [Folder]\n" + options : null;
      case DELETE_SHELF ->
          (arguments.size() != 3) ? "Usage: [option] [shelfID] [Folder]\n" + options : null;
      case DELETE_ROOM ->
          (arguments.size() != 3) ? "Usage: [option] [roomID] [Folder]\n" + options : null;
      case ADD_BOOK ->
          (arguments.size() != 7)
              ? "Usage: [option] [Title] [Author] [Genre] [BookCondition] [BookWidth] [Folder]\n"
                  + options
              : null;
