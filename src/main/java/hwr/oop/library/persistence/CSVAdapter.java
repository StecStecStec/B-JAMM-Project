    ...
    try (BufferedWriter writer = new BufferedWriter(new FileWriter(path + SHELF))) {
      for (Shelf shelf : shelfList) {
        writer.write(
            String.format(
                "%s;%s;%s;%d;%d%n",
                shelf.getShelfID().toString(),
                shelf.getRoomIn().getRoomID().toString(),
                shelf.getGenre(),
                shelf.getShelfWidth(),
                shelf.getBoardNumber()));
      }
    } catch (IOException e) {
        ...
        if (book.getShelf() == null) {
          writer.write(
              String.format(
                  ...
                  book.getBookGenre(),
                  "null",
                  book.getBorrowedBy().getVisitorID().toString()));
        } else {
          writer.write(
              String.format(
                  "%s;%d;%d;%s;%s;%s;%s;%s%n",
                  ...
                  book.getBookAuthor(),
                  book.getBookGenre(),
                  book.getShelf().getShelfID().toString(),
                  "null"));
        }
      }
    } catch (IOException e) {
    ...
    try (BufferedWriter writer = new BufferedWriter(new FileWriter(path + VISITOR))) {
      for (Visitor visitor : visitorList) {
        ...
        writer.write(
            String.format(
                "%s;%s;%s;%s;%s;%s;%s%n",
                visitor.getVisitorID().toString(),
                visitor.getVisitorName(),
                visitor.getVisitorSurname(),
                visitor.getVisitorBirthday(),
                visitor.getVisitorEmailAddress(),
                String.join(",", borrowedBookIdList),
                String.join(",", booksToReturnIdList)));
      }
    } catch (IOException e) {
    ...
    try (BufferedWriter writer = new BufferedWriter(new FileWriter(path + LIBRARIAN))) {
      for (Librarian librarian : librarianList) {
        writer.write(
            String.format(
                "%s;%s;%s;%s%n",
                librarian.getLibrarianID().toString(),
                librarian.getLibrarianName(),
                librarian.getLibrarianSurname(),
                librarian.getLibrarianBirthday()));
      }
    } catch (IOException e) {
  ...
  private void loadVisitor(Library library) throws FileNotFoundException {
    try (BufferedReader reader = new BufferedReader(new FileReader(path + VISITOR))) {
      ...
      while ((line = reader.readLine()) != null) {
        ...
        Visitor visitor =
            Visitor.createCompleteVisitor(library, name, surname, birthday, email, uuid);
        // borrowed books
      ...
      try (Stream<Path> paths =
          Files.find(
              ...
              (p, attr) ->
                  Files.isReadable(p)
                      && attr.isDirectory()
                      && p.getFileName().toString().equals(directoryNameToFind))) {
